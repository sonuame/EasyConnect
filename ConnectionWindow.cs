...
namespace EasyConnect
{
    ...
    public partial class ConnectionWindow : Form
    {
            ...
            : this()
        {
            ...
            urlTextBox.Text = ConnectionFactory.GetProtocol(connection).ProtocolPrefix + "://" + connection.Host + ":" + connection.Port;
        ...
        public async Task Connect()
        {
            ...
            _urlPanel.Text = String.Format(
                @"<div style=""background-color: #FFFFFF; font-family: {2}; font-size: {3}pt; height: {4}px; color: #9999BF;"">{0}://<font color=""black"">{1}:{5}</font></div>",
                ...
                _urlPanel.Height, _connection.Port);
            ...
            _suppressOmniBar = true;
            urlTextBox.Text = ConnectionFactory.GetProtocol(_connection).ProtocolPrefix + "://" + _connection.Host + ":" + _connection.Port;
            _suppressOmniBar = false;
            ...
            if (_validAutoCompleteEntries.Count > 0)
            {
                ...
                for (int i = 0; i < _validAutoCompleteEntries.Count; i++)
                {
                    ...
                    autoCompletePanel.Text =
                        String.Format(
                            @"<div style=""background-color: #FFFFFF; padding-left: 29px; padding-top: 5px; padding-bottom: 5px; padding-right: 5px; font-family: {3}; font-size: {4}pt; height: 30px; color: #9999BF;""><font color=""green"">{0}://{1}:{5}</font>{2}</div>",
                            ConnectionFactory.GetProtocol(connection).ProtocolPrefix,
                            Regex.Replace(connection.Host, urlTextBox.Text, "<b>$0</b>", RegexOptions.IgnoreCase),
                            connection.DisplayName == connection.Host ? "" : " - " + Regex.Replace(connection.DisplayName, urlTextBox.Text, "<b>$0</b>", RegexOptions.IgnoreCase),
                            urlTextBox.Font.FontFamily.GetName(0),
                            urlTextBox.Font.SizeInPoints,
                            connection.Port);
